					...
					<div class="box-body">
						...
						<form action="" method="POST" enctype="multipart/form-data">
							...
							<div class="form-group">
								<label>Nama</label>
								<input type="text" name="nama" placeholder="Nama Taman Budaya" value="<?= $d->nama ?>" class="input-control" required>
							</div>

							<div class="form-group">
								<label>Email</label>
								<input type="email" name="email" placeholder="Email Taman Budaya" value="<?= $d->email ?>" class="input-control" required>
							</div>

							<div class="form-group">
								<label>Telepon</label>
								<input type="text" name="telp" placeholder="Telepon Taman Budaya" value="<?= $d->telepon ?>" class="input-control" required>
							</div>
							...
							<div class="form-group">
								<label>Logo</label>
								<img src="../uploads/test/<?= $d->logo ?>" width="200px" class="image">
							...
							<div class="form-group">
								<label>Favicon</label>
								<img src="../uploads/test/<?= $d->favicon ?>" width="32" class="image">
							...
							if(isset($_POST['submit'])){
								...
								if($_FILES['logo_baru']['name'] != ''){
									...
									if(!in_array($formatfile_logo, $allowedtype_logo)){
										...
										if(file_exists("../uploads/test/".$_POST['logo_lama'])){

											unlink("../uploads/test/".$_POST['logo_lama']);

										}

										move_uploaded_file($tmpname_logo, "../uploads/test/".$rename_logo);
								...
								if($_FILES['favicon_baru']['name'] != ''){
									...
									if(!in_array($formatfile_favicon, $allowedtype_favicon)){
										...
										if(file_exists("../uploads/test/".$_POST['favicon_lama'])){

											unlink("../uploads/test/".$_POST['favicon_lama']);

										}

										move_uploaded_file($tmpname_favicon, "../uploads/test/".$rename_favicon);
								...
								if($update){
									echo "<script>window.location='identitas-taman-budaya.php?success=Edit Data Berhasil'</script>";
								}else{
